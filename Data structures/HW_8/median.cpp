...
std::vector<double> findMedian(std::vector<int>& data) {
    ...
    max_heap.push(0); // Code crashes without these.
    min_heap.push(100000000);

    // need to see all input
    for (int i = 0; i < data.size(); i++) {
        // add input to stack
        if ((data.at(i) >= max_heap.top()) && (data.at(i) <= min_heap.top())) {
            max_heap.push(data.at(i));
        } else {
            min_heap.push(data.at(i));
        }

        MinMax = min_heap.size() - max_heap.size();
        MaxMin = max_heap.size() - min_heap.size();
        ...
        if (max_heap.size() == min_heap.size()) {
            res.push_back((min_heap.top() + max_heap.top()) / 2.0);
