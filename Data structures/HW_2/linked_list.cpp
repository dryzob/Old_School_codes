...
//
/*
 Concatenate two input singly linked lists l1 and l2 into a new output linked list that contains all the nodes of
...
void concat(SLinkedList<T>& l1, SLinkedList<T>& l2, SLinkedList<T>& lout) {
	SNode<T>* iter = l1.head;
	while (iter != nullptr) {
		lout.addBack(iter->elem);
		iter = iter->next;
	}
	iter = l2.head;
	while (iter != nullptr) {
