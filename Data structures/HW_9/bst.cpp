...
private:
      ...
      std::string findHelper(Node* p, int key) const; // Helper for recursive find()
};
...
std::string BinarySearchTree::find(int key) const {
   Node* p = root;
   return findHelper(p, key);
}

std::string BinarySearchTree::findHelper(Node* p, int key) const {
   if (p == nullptr) {
      return "";
   } else if (p->key == key) {
      return p->val;
   } else if (p->key > key) {
      return findHelper(p->left, key);
   } else {
      return findHelper(p->right, key);
   }
}
