...
void DFSHelper(const adjlist& alist, vector<int>& dfslist, vector<bool>& visited, int source) {
    ...
    for (auto e : alist[source]) {
        DFSHelper(alist, dfslist, visited, e.first);
    }
}
...
vector<int> BFS(const adjlist& alist, int source) {
    ...
    q.push(source);
    visited[source] = true; // visited starting one
    while (!q.empty()) {
        bfsList.push_back(q.front()); // build list
        for (auto e : alist[q.front()]) {
            if (!visited[e.first]) { // see if already visited
                visited[e.first] = true;
                q.push(e.first);
            }
        }
        q.pop(); // depopulate queue
    }
    return bfsList;
}
